...
`ggbiplot()` only requires one argument, an `object` returned by a function performing PCA. Additional arguments help customize the output for visual clarity and data analysis. For example, the `groups` argument classifies each data point by a variable in the original data set, allowing the user to see whether different clusters form on the new PC axes. 
